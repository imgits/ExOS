...
using EFI_EVENT_NOTIFY = EFIAPI VOID (*)
(EFI_EVENT Event,
 VOID *Context);

using EFI_TPL = UINTN;

using EFI_CREATE_EVENT = EFIAPI EFI_STATUS (*)
(UINT32 Type,
 EFI_TPL NotifyTpl,
...
using EFI_CREATE_EVENT_EX = EFIAPI EFI_STATUS (*)
(UINT32 Type,
 EFI_TPL NotifyTpl,
 EFI_EVENT_NOTIFY NotifyFunction,
 const VOID *NotifyContext,
 const EFI_GUID *EventGuid,
 EFI_EVENT *Event);
...
using EFI_CLOSE_EVENT = EFIAPI EFI_STATUS (*)
(EFI_EVENT Event);

using EFI_SIGNAL_EVENT = EFIAPI EFI_STATUS (*)
(EFI_EVENT Event);

using EFI_WAIT_FOR_EVENT = EFIAPI EFI_STATUS (*)
(UINTN NumberOfEvents,
 EFI_EVENT *Event,
 UINTN *Index);

using EFI_CHECK_EVENT = EFIAPI EFI_STATUS (*)
(EFI_EVENT Event);
...
using EFI_SET_TIMER = EFIAPI EFI_STATUS (*)
(EFI_EVENT Event,
 EFI_TIMER_DELAY Type,
 UINT64 TriggerTime);

using EFI_RAISE_TPL = EFIAPI EFI_TPL (*)
(EFI_TPL NewTpl);
...
using EFI_RESTORE_TPL = EFIAPI VOID (*)
(EFI_TPL OldTpl);
