...
EFI_STATUS Uefi::get_gop(EFI_HANDLE handle, const EFI_BOOT_SERVICES &bs, EFI_GRAPHICS_OUTPUT_PROTOCOL *&gop)
{
    ...
    EFI_STATUS status = bs.LocateHandleBuffer(ByProtocol, &gop_guid, nullptr, &num, &handles);
    if (status_is_error(status))
        return status;

    for (UINTN i = 0; i < num; ++i) {
        status = bs.OpenProtocol(handles[i], &gop_guid, reinterpret_cast<VOID **>(&gop),
                                 handle, nullptr, EFI_OPEN_PROTOCOL_BY_HANDLE_PROTOCOL);
        if (status_is_error(status))
            return status;
...
void Uefi::die(EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL &conout, EFI_STATUS status, StringRefUefi s)
{
    print(conout, s);
    print(conout, u": "_s);
